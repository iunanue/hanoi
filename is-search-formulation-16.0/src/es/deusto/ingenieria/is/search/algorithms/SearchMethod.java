package es.deusto.ingenieria.is.search.algorithms;
...
import es.deusto.ingenieria.is.search.algorithms.log.SolutionLog;
...
public abstract class SearchMethod {
	...
	public void createSolutionLog(List<String> operators) {		
		try {
			...
			SolutionLog solutionLog = new SolutionLog("log/" + this.getClass().getSimpleName() + "_Solution " +
					                                  formatter.format(Calendar.getInstance().getTime()) + ".log", operators);
			solutionLog.close();
		} catch (Exception ex) {
			System.err.println("% createSolutionLog(): " + ex.getMessage());
		}
	}	
	 ...
	 */
	public SearchLog createSearchLog() {
		SearchLog log = null;

		try {
			SimpleDateFormat formatter = new SimpleDateFormat("(dd-MM-yyyy@HH_mm_ss)");
			//create log for the search process.
			log = new SearchLog(this.getClass().getSimpleName()+ " " +
					             formatter.format(Calendar.getInstance().getTime()) + ".log");
		} catch (Exception ex) {
			System.err.println("% createSearchLog(): " + ex.getMessage());
		}
	 ...
	 */
	public void writeInSeachLog(SearchLog log, List<Node> nodelist) {
		if (log != null) {
	 ...
	 *            SearchLog representing the log file. 
	 */
	public void closeSearchLog(SearchLog log) {
		if (log != null) {
			log.closeLog();
		}
	}
